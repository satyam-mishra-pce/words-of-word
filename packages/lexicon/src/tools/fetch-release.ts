...
async function main(): Promise<void> {
  ...
  const bundled=resolve(dirname(manifestPath),`${manifest.fileName}.gz`);
  if (existsSync(bundled)) {
    await pipeline(createReadStream(bundled),createGunzip(),createWriteStream(temporary));
  } else {
    if (!manifest.downloadUrl) throw new Error('No bundled artifact or release URL is configured.');

...
test('built artifact has exact ordered legacy inventory when required',()=>{ const path=resolve('artifacts/words-of-word-lexicon-v0.1.0.sqlite'); if(process.env.REQUIRE_LEXICON_ARTIFACT==='1' && !existsSync(path)) assert.fail(`Required artifact missing: ${path}`); if(!existsSync(path)) return; const manifest=JSON.parse(readFileSync(resolve('artifacts/manifest.json'),'utf8')); assert.deepEqual(loadPlayableWords({path,expectedArtifactVersion:manifest.artifactVersion,expectedSha256:manifest.sha256}),require('an-array-of-english-words')); });
...
test('two shards from one run merge once, preserve immutable output, and reject tampered derived rows',()=>{
  ...
  try {
    ...
    const output=join(dir,'merged.sqlite'); const result=mergeShards({sourceArtifact:artifact,outputArtifact:output,workPaths:[first.path,second.path]});

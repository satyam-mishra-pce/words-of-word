...
function fixture(): {dir:string;path:string} {
  ...
  for (const [key,value] of Object.entries({artifact_version:'test-1',release_status:'complete',missing_definition_count:'0',source_word_count:'1',eligible_word_count:'1',eligible_words_sha256:createHash('sha256').update('test\n').digest('hex')})) db.prepare('INSERT INTO artifact_metadata VALUES (?,?)').run(key,value);
...
test('runtime refuses a partial artifact so 0.1.0-style data can never be served',()=>{ const f=fixture(); try { const db=new Database(f.path); db.prepare("UPDATE artifact_metadata SET value='partial' WHERE key='release_status'").run(); db.prepare("UPDATE artifact_metadata SET value='154816' WHERE key='missing_definition_count'").run(); db.close(); const sum=checksum(f.path); assert.throws(()=>loadPlayableWords({path:f.path,expectedArtifactVersion:'test-1',expectedSha256:sum}),/release_status/); assert.throws(()=>openDefinitionStore({path:f.path,expectedArtifactVersion:'test-1',expectedSha256:sum}),/release_status/); } finally { rmSync(f.dir,{recursive:true,force:true}); } });
